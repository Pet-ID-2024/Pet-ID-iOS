...
extension LoginMainViewModel {
    ...
    func requestOAuthLogin(
        ...
        //        currentOAuth = oauth // 현재 OAuth 저장
        Task {
            do {
                let isFirstLogin = UserDefaultManager.shared.isFirstExecute
                logger.debug("isFirstExecute before login: \(isFirstLogin)")
                let result = try await loginUseCase.execute(oauth: oauth, fcmToken: fcmToken)
                
                if result {
                    Logger().debug("Login successful")
                    if isFirstLogin {
                        Logger().info("First login detected. Redirecting to Terms Agreement.")
                        self.result.send(.terms(oauth)) // 첫 로그인 시 약관 동의 화면으로 이동
                    } else {
                        Logger().info("Normal login. Redirecting to Main.")
                        self.result.send(.main) // 메인 화면으로 이동
                    }
                } else {
